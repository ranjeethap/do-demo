...
# --- Ingress address discovery (IP or hostname) ---
get_ingress_addr() {
  local ns="${1:-ingress-nginx}"
  local svc="${2:-ingress-nginx-controller}"
  local addr=""

  echo "Waiting for external address of ${ns}/${svc} ..."
  for i in {1..60}; do
    addr="$(kubectl -n "$ns" get svc "$svc" \
      -o jsonpath='{.status.loadBalancer.ingress[0].ip}{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null | tr -d ' ')"
    if [[ -n "$addr" ]]; then
      echo "Found ingress address: $addr"
      break
    fi
  ...
  if [[ -z "$addr" ]]; then
    echo "ERROR: no external address for ${ns}/${svc}" >&2
    return 1
  fi
...
# --- Render dev/prod manifests by substituting __ING_ADDR__ ---
render_manifests() {
  local src_dir="scripts/manifests"
  ...
  for f in "$src_dir"/dev-*.yaml "$src_dir"/prod-*.yaml; do
    [[ -f "$f" ]] || continue
    local base; base="$(basename "$f")"
    sed "s/__ING_ADDR__/${ING_ADDR}/g" "$f" > "$out_dir/$base"
  done
